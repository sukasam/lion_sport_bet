...
$query = mysqli_query($conn, $sql) or die("deposit_pm_all_ajax.php: get deposit_history");
$totalData = mysqli_num_rows($query);
$totalFiltered = $totalData; // when there is no search parameter then total number rows = total number filtered rows.

$sql = "SELECT *";
$sql .= " FROM deposit_history WHERE 1=1 AND deposit_type = 'PM'";
...
while ($row = mysqli_fetch_array($query)) { // preparing an array
    ...
    //$nestedData[] = "<center class=\"btAction\"><button class=\"btn btn-primary btn-xs\" title=\"Edit\" onClick=\"window.location='deposit_pm_all_edit.php?Player=".$row["Player"]."'\"><i class=\"fa fa-pencil\"></i></button><button class=\"btn btn-warning btn-xs\" title=\"Balance\" onClick=\"window.location='deposit_pm_all_balance.php?Player=".$row["Player"]."';\"><i class=\"fa fa-btc\"></i></button><button class=\"btn btn-success btn-xs\" title=\"Bank Info\" onClick=\"window.location='deposit_pm_all_bank.php?Player=".$row["Player"]."';\"><i class=\"fa fa-building-o\"></i></button><button class=\"btn btn-info btn-xs\" title=\"Invite\" onClick=\"window.location='deposit_pm_all_invite.php?Player=".$row["Player"]."';\"><i class=\"fa fa-users\"></i></button></center>";
    //$nestedData[] = "<center class=\"btAction\"><button class=\"btn btn-primary btn-xs\" title=\"Edit\" onClick=\"window.location='deposit_pm_all_edit.php?Player=".$row["Player"]."'\"><i class=\"fa fa-pencil\"></i></button><button class=\"btn btn-warning btn-xs\" title=\"Balance\" onClick=\"window.location='deposit_pm_all_balance.php?Player=".$row["Player"]."';\"><i class=\"fa fa-btc\"></i></button></center>";
