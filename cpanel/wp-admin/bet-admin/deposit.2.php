<?php  
    ...
    if($_GET['action'] == "submit"){
      ...
      if($_REQUEST['status'] == "1"){
        $addChipPromo = $_REQUEST['amount']+500;
    ...
    $RecData = $db->select("SELECT * FROM deposit_history WHERE deposit_type = 'Online Card' ORDER BY id DESC");
    $configDT = $db->select("SELECT currency FROM setting WHERE sid ='1' ORDER BY sid DESC");
    $RecDataSum = $db->select("SELECT sum(amount) as sumAmount FROM deposit_history WHERE deposit_type = 'Online Card' AND status = '1'");
  ...
  <section id="container">
    ...
    <section id="main-content">
      <section class="wrapper">
        <h3><i class="fa fa-angle-right"></i> Deposit (Online Card)</h3>
                ...
                <tbody>
                    ...
                    foreach ($RecData as $key => $value) {
                      if($value['deposit_type'] == "E-Voucher"){
                        $amountBl = $value['amount'] * $configDT[0]['currency'];
                      }else{
                        ...
                        <tr role="row">
                            ...
                            <td class="text-center"><?php if($value['status'] == 1){echo '<button class="btn btn-success" style="width: 120px;">Complated</button>';}else if($value['status'] == 2){echo '<button class="btn btn-danger" style="width: 120px;">Payment failed</button>';}else{echo '<button class="btn btn-primary" style="width: 120px;">Processing</button>';}?></td>
                            <td class="center">
                              <?php 
                              if($value['deposit_type'] == "Online Card" && $value['status'] != 1){
                                ?>
                                <div class="btn-group">
                                  ...
                                  <ul class="dropdown-menu" role="menu" style="left: -81px;">
                                    <!-- <li><a href="deposit2.php?action=submit&status=0&statusOld=<?php echo $value['status'];?>&id=<?php echo $value['id']?>&Player=<?php echo $value['player'];?>&amount=<?php echo $amountBl;?>">Processing</a></li> -->
                                    <li><a href="deposit2.php?action=submit&status=1&id=<?php echo $value['id']?>&Player=<?php echo $value['player'];?>&amount=<?php echo $amountBl;?>">Complated</a></li>
                                  </ul>
                                </div>
                                <?php
                              }
