<?php
   ...
   if($api->PRake != 0){
	...
	/////////////////////////
		...
		$rth=(strval(intval($RecTophand[0]['count'])*9856));
		$rth=base64_encode($rth)."QkYXHax";
		$rtl=(strval(intval($RecToplost[0]['point'])*8569));
		$rtl=base64_encode($rtl)."Q8BW)tH";
		$rdp=(strval(floatval($countPRake)*546));
		$rdp=base64_encode($rdp)."QEw71eup";
		$str=$plyer."-".$rth."-".$rtl."-".$rdp;
		$_SESSION['tStHtLtW']=base64_encode($str);
