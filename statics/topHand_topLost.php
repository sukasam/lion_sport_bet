    ...
    for($j=0;$j<$api->Files;$j++){
      ...
      if($api->Date[$j] === $dateCerrent || $api->Date[$j] === $dateBefore){
        ...
        $postfields = array(
            ...
            'logs'=>'getcount_autoscript'
        );
        
       $responseCount = curl_post(SCRIPT_AUTO_SITE,$postfields);
        ...
        $updateCount = ($responseCount + $api2->Count);

        if($updateCount > 30){
          $updateCount = $updateCount-30;
        }

        $postfields = array(
            'date'=> $api->Date[$j],
            'table_game'=> $api->Name[$j],
            'rec_count'=> $updateCount,
            'logs'=>'autoscript'
        );

        $responseUpdateAutoScript = curl_post(SCRIPT_AUTO_SITE,$postfields);
